...
export class AdminService {
  ...
  async createWhatsAppFlow(
    tenantId: string,
    name: string,
    // Set when instantiating from a library template (see the frontend's
    // static FLOW_TEMPLATES) instead of starting from a blank canvas —
    // still just a normal editable flow afterward, no different from one
    // built by hand or generated by AI.
  ...
  // Runs one turn of the CURRENT (possibly unsaved) canvas definition
  // through the exact same channel-agnostic engine WhatsApp/the app use —
  // so "yes, this is what I want" can be confirmed before ever saving or
  // going live. Fully stateless on this end: the caller (the flow editor
  // page) holds the conversation's sessionState across calls and sends it
  // back each turn, rather than this persisting a real session row.
  // Platform-data/prescription nodes still do REAL reads/writes (booking a
  // slot, uploading a prescription) — there's no separate "dry run" mode —
  // but they run against one dedicated, clearly-labeled test patient per
  // tenant rather than a real one.
  async previewWhatsAppFlow(
    tenantId: string,
    definition: WhatsAppFlowDefinition,
    text: string,
    sessionState?: {
    ...
    const previewPatient = await this.getOrCreatePreviewPatient(tenantId);

    const session = AppDataSource.getRepository(AppFlowSession).create({
      userId: previewPatient.id,
      tenantId,
      awaitingHuman: false,
      messages: sessionState?.messages ?? [],
      flowVariables: sessionState?.flowVariables ?? {},
      activeFlowId: sessionState?.activeFlowId ?? 'preview',
      flowNodeId: sessionState?.flowNodeId ?? null,
    });

    const previewFlow = AppDataSource.getRepository(WhatsAppFlow).create({
      tenantId,
      name: '__preview__',
      isActive: false,
      definition,
    });
    previewFlow.id = session.activeFlowId ?? 'preview';

    const { result, steps } = await this.flowEngine.processAppTurn(
      session,
      text,
      previewFlow,
    );
    ...
    return {
      ...
      steps,
      sessionState: {
        flowNodeId: session.flowNodeId ?? null,
        activeFlowId: session.activeFlowId ?? null,
  ...
  private async getOrCreatePreviewPatient(tenantId: string): Promise<User> {
    const userRepo = AppDataSource.getRepository(User);
    const firebaseUid = `flow-preview-${tenantId}`;
    let user = await userRepo.findOne({ where: { firebaseUid } });
    if (!user) {
      user = userRepo.create({
        firebaseUid,
        fullName: '[Flow Preview] Test Patient',
